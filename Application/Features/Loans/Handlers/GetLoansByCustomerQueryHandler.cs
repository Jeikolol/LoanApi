...
{
    public class GetLoansByCustomerQueryHandler : IRequestHandler<GetLoansByCustomerQuery, PaginatedResponse<LoanSummaryResponse>>
    {
        private readonly IDbConnection _connection;
        private readonly ILogger<GetLoansByCustomerQueryHandler> _logger;

        public GetLoansByCustomerQueryHandler(IDbConnection connection, ILogger<GetLoansByCustomerQueryHandler> logger)
        {
            _connection = connection;
            _logger = logger;
        }

        public async Task<PaginatedResponse<LoanSummaryResponse>> Handle(GetLoansByCustomerQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Getting loans for customer {CustomerId} - Page: {PageNumber}, Size: {PageSize}",
            ...
            const string query = @"
                ...
                WHERE L.CustomerId = @CustomerId AND L.IsDeleted = 0
            ...
            var args = new { request.CustomerId, Skip = request.GetSkip(), Take = request.PageSize };
