...
{
    public class GetDocumentsByLoanQueryHandler : IRequestHandler<GetDocumentsByLoanQuery, PaginatedResponse<LoanDocumentResponse>>
    {
        private readonly IDbConnection _connection;
        private readonly ILogger<GetDocumentsByLoanQueryHandler> _logger;

        public GetDocumentsByLoanQueryHandler(IDbConnection connection, ILogger<GetDocumentsByLoanQueryHandler> logger)
        {
            _connection = connection;
            _logger = logger;
        }

        public async Task<PaginatedResponse<LoanDocumentResponse>> Handle(GetDocumentsByLoanQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Getting documents for loan {LoanId}", request.LoanId);
